...
const selectApprovedFarms = `SELECT id, name, description, sub_account_name, rewards_from_pool_btc_wallet_name, total_farm_hashrate, address_for_receiving_rewards_from_pool, leftover_reward_payout_address, maintenance_fee_payout_address, maintenance_fee_in_btc FROM farms WHERE status='approved'`
